  ...
  function(genome, md=NULL)
  {
    ...
    browser=paste(geneChr,geneStarts, sep=":") #coordinates for genome browser
      gene_coordinates=paste(browser,geneEnds, sep="-")
    #agrego el shareLocus
    if (packageVersion("IRanges")<2.6) 
      {
        locus=findOverlaps(exons.by.gene.disjoint, ignoreSelf=TRUE) #locus by locus
      }
    else {
      locus=findOverlaps(exons.by.gene.disjoint, drop.self=TRUE) #locus by locus  
    }  
...
.createGRangesExons <-
  function(genome, md) {
    ...
    #add metadata
    #add locus
    mcols(exon.bins) <- append(mcols(exon.bins), DataFrame(locus=exon.gene.names.repeated) )
    #add gene coord
    ...
    symbol=md[tt, ]
    mcols(exon.bins) <-append(mcols(exon.bins), DataFrame(symbol=symbol))
    return (exon.bins)
  }
########################################################
.createGRangesIntrons <-
  function(genome, md=NULL)
  {
    ...
    length(intron.o)
    #add metadata
    intron.o.num <- lapply(intron.o.gene.names,function(x){seq(1:x)}); length(intron.o.num)
    intron.o.num <- unlist(intron.o.num); length(intron.o.num)
    intron.o.num<-sprintf('Io%03d',  intron.o.num); length(intron.o.num)
    feature.o.intron<-rep("Io", length(intron.o.num) );length(feature.o.intron)
    length(intron.o)
    ...
    length(unique(intrones.totales))
...
#########################################################  
  .findASBin <-
    function(exon.bins, intron.bins, transcripts, junctions)
    {
      ...
      #exons.introns.unique<-exons.introns.sort
