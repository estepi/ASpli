...
.counterGenes <-
  function(reads, feature, cores=NULL)
  {
    ...
    #strand<-sapply(strand(feature),unique)
...
.ovBinJunction<-function(features, jranges)
{
  ...
  posJrange=overGeneDF$queryHits #replace index numbers by names; posJrange[1:10]
  posGene=overGeneDF$subjectHits#replace index numbers by names; posGene[1:10]
  ...
  j_within_bin=agtt$subjectHits[tw]; head(j_within_bin)
